...
export class D3MapComponent {
  ...
  globalRegionData:any = [{'us-east-1':1}]
  // globalRegionData:any = [1,2]
  ...
  ngAfterViewInit(): void {
      this.setup();
      this.initSvg();
      // this.parseD3Data(this.appcomponentdata);
      this.clearSvg();
      this.getProjection();
      this.getPath();
      this.getTooltip();
      this.drawMap(this.appcomponentdata);
      this.globalRegionData=this.appcomponentdata;
  }
          ...
          this.svg.selectAll("circle")
              ...
              .on("mouseover", (d) => {
                  let tooltext = d.properties.name + "<br/>"
                      + "Region : " + d.id + "<br/>";
                  if (data.hasOwnProperty(d.id)) {
                      `<span color="primary">hiiiiii</span>`
                      tooltext = 
                        `<span color="primary"><b>${d.properties.name}</b>, <b>Region: </b>  ${data[d.id].name}, <b>Cost:</b> ${data[d.id].totalcost}</span>`
                        //   + "Region : " + data[d.id].name + " "
